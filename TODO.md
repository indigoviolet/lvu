...
| **Ready** | The merged-view order extends in O(k) per refresh instead of rebuilding: keys and order both resume from the tail when every arriving key is strictly newer (interleaved) or only the last source grew (concatenated), else rebuild; the fast path is compared against the slow one directly. W24's refresh-cost test is un-ignored: 3.0x → ~1.0x for 8x the view under load. (W15, `fc46f42`, in gate) |
