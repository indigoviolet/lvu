...
| **Ready** | In the Fields dialog `x`, `F`, `d` do nothing: the underlined letters are Alt mnemonics only. The underlined letter is now the key, whenever no text field has focus; Alt is the fallback for when one does. Resolved once in the shell's key dispatch, so every dialog gets it. Alt was never reliably deliverable: an xterm with its default `metaSendsEscape: false` sends Alt-f as the character `æ`. `lvu --keys` prints what a terminal actually sends. (W27) |
| **Ready** | Help (`?`) should not list command-line options; `lvu --help` and the README own the CLI. Remove the `--fresh`/`--resume` lines and say so in the Help rule. (W27) |
...
| --- | --- |
<<<<<<< HEAD
| **Working** | Incremental refresh: membership publishes as merging immutable chunks, so a capture refresh adds a chunk instead of rebuilding; refresh CPU 0.207 → 0.071 per second at 620k and flat across view sizes. Whole-view field statistics: engine and worker done (Polars aggregations, bounded, cancellable, superseded); app wiring, Value pane heading and PTY story remaining. Columnar-cache design note written. (W24, `perf/incremental-refresh`, in its gate) |
| **Ready** | Capture committed every few scheduler batches (23 fsyncs per MB, plus one per checkpoint), so under contention it waited ~460 ms per MB on fsync. Group commit now: one commit per 4,096 records or 500 ms, the file cursor written only behind the commit that covers it; 2.5 commits per MB, capture settled 66 → 39 s on the soak, shutdown 0.9 → 0.1 s; a guard asserts commits per MB. Next: framing and encoding cost 36 ms/MB against 2.4 ms to read. (W25, `ca0e519`, in gate) |
||||||| parent of ca0e519 (Commit capture by the record, not by however many happened to be queued)
| **Ready** | Capture committed every few scheduler batches (23 fsyncs per MB, plus one per checkpoint), so under contention it waited ~460 ms per MB on fsync. Group commit now: one commit per 4,096 records or 500 ms, the file cursor written only behind the commit that covers it; 2.5 commits per MB, capture settled 66 → 39 s on the soak, shutdown 0.9 → 0.1 s; a guard asserts commits per MB. Next: framing and encoding cost 36 ms/MB against 2.4 ms to read. (W25, `ca0e519`, in gate) |
=======
...
| **Open** | `mise run soak` stops in the restarted app instance: `timed out waiting for screen containing 'Fields · record'`, with Fields reporting "No record selected" over a viewport that is showing rows. A record-scoped operation has no selection while the source is still being appended to, which is the FOLLOW-selection row above seen from the soak. Reproduced identically on `3550c7e` and on `perf/ingest-throughput`, so it gates every soak run until it is fixed. Evidence: `/tmp/soak-before.log` shape, both runs reach cycle 3 of 6. |
>>>>>>> ca0e519 (Commit capture by the record, not by however many happened to be queued)
