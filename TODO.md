...
Current published version: **v0.1.16**. See [CHANGELOG.md](CHANGELOG.md) for what
...
| Status | Work |
| --- | --- |
| **Working** | Make automatic log setup explicitly inspectable at every lifecycle stage: the current candidate adds `Current log > Setup status`, a visible retry, and a durable session identity while preserving raw-first behavior. Finish full candidate acceptance before versioning the next normal release. |
...
| **Open** | Consider recent display-zone choices after the System timezone, UTC and searchable IANA picker. UTC remains the stored default; named zones keep daylight-saving behavior and fixed offsets remain valid compatibility values. |
