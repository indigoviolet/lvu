...
| **Working** | Soak and slow-terminal harnesses: `mise run soak`, `soak:long`, `soak:slow-terminal`. The slow-terminal story passes. The soak generates the workload, captures it, runs a full cycle of filters, surfaces and restarts, and records RSS, capture and index growth, query latency, input-loop and shutdown timings — but its second cycle stops finding the Search surface after the first cycle opens Details, Fields and Bookmarks, so multi-cycle runs do not complete. Needs one hardening pass on a quiet box: find which surface leaves focus somewhere `/` is not the search key. |
| **Open** | Filtering waits about 60,000 records per second and uses almost no CPU. Measured after capture settled, on an otherwise idle 16-core box: 152k records take 2.42s and 620k take 10.75s (p50), the same within 2% whether the filter matches 7 rows or most of them, at 0.18s of CPU across 10.57s of wall clock. So it is not scan throughput and not the machine — something blocks per record or per batch. A 3 GB source would be roughly eight minutes per filter. Evidence: `mise run soak`, and `tests/soak/soak.py` records p50/p99 per run. |
| **Open** | Capture reads about 10 MB/s: a 64 MB generated source takes four to five minutes before the first rows appear, and its journal is 108 MB for 64 MB of input. Worth separating read cost from journal write cost against a plain `cat` of the same file. Evidence: every `mise run soak` sample line. |
