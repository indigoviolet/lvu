...
Current app: **preview 054**. This is the single feedback and work list.
...
| **Working** | A merged multi-source view interleaves by time (W15, eight invariants tested). Decided: interleaving follows the basis; a capture-time basis keeps the user's explicit source order because capture times for files read together are scheduling accidents, while event, extracted and chosen-column bases merge by time; the order row says which. A grouped view merges groups, not records. Follow-up: extend the order in O(k) per refresh once W24's `Appended` refactor lands. |
...
| Status | Request |
| --- | --- |
| **Ready** | Quitting lvu while a short-lived command source was finishing reported `source is not active` and exited non-zero: shutdown snapshotted live sources, then treated a source that had ended on its own as a failure to stop. Shutdown now waits for the source's own terminal state and reports its outcome; 30/30 under load where 7/10 failed before. Found through W25's resume test. (W25, `a056eeb`, queued) |
