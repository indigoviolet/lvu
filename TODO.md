...
| **Done** | Restarting a source right after `stop()` reports `complete` no longer fails with `Journal(AlreadyOpen)`. The journal lock was an `flock`, which belongs to the open file description, and `fork` hands every child a duplicate: between the fork and the `exec` that closes it, a child spawned by a command source holds the lock, so a journal dropped in that window stays locked after it is gone. `configure_owned_process`'s `pre_exec` widens the window by taking std off `posix_spawn`. Measured: the 200-cycle restart loop with four threads spawning subprocesses failed 11 runs in 20 at load 23, the same loop without them 0 in 20; after the fix, 0 in 20 with the spawners at load 27. Ownership is now split — an in-process registry of claimed lock paths for exactness, and a POSIX record lock (`F_SETLK`), which `fork` does not pass to the child, across processes. The reproducer is `mise run test:restart-race` and is back in the default gate. |
| **Open** | The same `flock`-across-`fork` hazard applies to every other lock in the tree — `runtime.lock` in `lvu-ingest`, `.lvu-index-ownership.lock` in `lvu-live`, the memory store's `.write.lock`, settings and storage ownership in `lvu-app`. None has been seen to fail, and each needs its own reproducer before it is changed; the journal fix is the pattern to copy. |
