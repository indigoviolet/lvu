...
| **Working** | Automatic shared capture: combined candidate `6c806ed` passes owner-reported same-cache two-window and sequential shutdown/reopen acceptance. The second-window blank-view blocker was an exclusive derived-index lock; window-specific overflow indexes fix contention without duplicating capture. Independent review of the final index lifecycle and combined controller remains pending before primary integration. |
| **Working** | Shared-worker admission dependency `4a7cd29` is independently source- and evidence-clear: 111 library and five real-worker tests pass. Combined controller includes it; primary integration awaits the final controller review. Superseded candidate failures remain in the ledger and retained logs. |
