...
| **Open** | Restarting a source right after `stop()` reports `complete` fails with `Journal(AlreadyOpen)`, about one full-workspace run in eight under load. The writer is exonerated: with `LVU_JOURNAL_TRACE=1` the ring buffer shows `journal DROPPED fd=94` → `run_writer RETURN ok=true` → `source_task JOINED writer ok=true` → `STOP REPLY`, and the refusal arrives 5.2 ms later on the same source id, so the only `Journal::open` in the product had already released. An earlier `/proc` scan found a descriptor on the lock path owned by this pid at fd 11 — a different number from the one the journal held — which says a *duplicate* of that open file description survived it; `flock` is only released when the last descriptor closes. The remaining suspect is the fork window: every `Command` spawn copies the descriptor table into the child until `exec`, and a workspace run spawns constantly. Unconfirmed — `holder_report` now reads `/proc/locks` to name the owning pid, and has not yet caught a failure. Reproduce: `LVU_JOURNAL_TRACE=1 cargo test --workspace -- --include-ignored` under load, or `mise run test:restart-race`. |
