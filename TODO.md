...
| Status | Request |
| --- | --- |
| **Working** | Design spike, no code: a derived, disposable columnar cache (Arrow IPC or Parquet per journal segment) for hot columns beside the raw-bytes journal, with invalidation, expected gain at 620k/3M/30M records, disk cost and the threshold at which it pays. docs/columnar-cache.md. (W24, after its queue) |
