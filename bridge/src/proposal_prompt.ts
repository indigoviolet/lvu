...
function proposalKindInstructions(kind: ProposalRequest["kind"]): string[] {
  ...
  switch (kind) {
    ...
    case "auto_setup": return [
      ...
      "When recognizing severity, name the output severity and normalize observed levels to TRACE, DEBUG, INFO, WARN, ERROR or FATAL; unknown values should remain null. When recognizing event time, name its UTC output timestamp_utc. These output names let the host offer the corresponding display roles; they are never assumptions about input field names. Outputs must be scalar values, not lists or structs.",
      temporal,
