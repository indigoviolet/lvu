...
**State: `v0.1.11` is the latest published release, with the supported
...
recognize the bridge's new coded daemon-unreachable diagnostic, and it was not
published or given assets.**
...
`v0.1.11` is published. `v0.1.7` remains an immutable unpublished draft because
...
use a new normal version; never move or reuse `v0.1.7` through `v0.1.11`, and
