...
| 11 | Fields `i` | component | `Pin`/`Unpin` `Color rows by field` `Correlate across sources` (or `Raw context` when the record has no fields) | index 0 | list: pins (Space too) | buttons: themselves | list 0 ✓ | **Pin** (filled) | fill only |
