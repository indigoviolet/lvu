...
| 8 | Sources `N` / Add source `n` | component | Sources: `Add source` `Restart` `Remove`; Add source: `Open` / `Request proposal` / `Start reviewed source` (+ `Rescan` in Discover) | Sources defaults to Restart (index 1); Remove is never default. Add source defaults to index 0 | Sources list: restarts highlighted source; Path/Filter/Describe field: submits; a highlighted suggestion opens it | segments/radios/Rescan: themselves (§8.9 exception) | source/suggestion/candidate row 0 (verified) | `Restart` or the add method's primary verb (filled) | Add source is a separate screen; Escape returns to Sources when reached from its action |
| 9 | View `v` | component | `Apply`/`Apply membership`/`Delete view`; `New blank │ Clone │ Rename │ Sources │ Delete` is the header control | **`New blank`** was styled primary (index 0) although Apply is the verb | Name field: applies | Sources list: applies; header segments switch mode without submitting | membership list row 0 (verified) | **Apply** (filled, the sole action); **Delete view** is destructive and never filled — Delete opens on the header, the first press arms reversibly, the second submits, and canonical/union refusals keep the dialog open | Modes moved into the shared segmented header. One header focus stop replaces four faux buttons; `render_action_row` draws only the contextual verb. Also fixed: at 80x24 the old gutter mismatch dropped `[ Apply ]` off the screen entirely. Delete is the reasoned exception to "one default": its two-press arm replaces the fill as the safety mechanism. |
