...
| 8 | Sources / Add source `n` | component | Existing: `Restart` `Remove`; Manual/Discover/Agent: `Open` / `Request proposal` / `Start reviewed source` (+ `Rescan` in Discover) | index 0; Remove is never default | Existing list: restarts; Path/Filter/Describe field: submits; a highlighted suggestion opens it | segments/radios/Rescan: themselves (§8.9 exception) | existing/suggestions/candidates 0 (verified) | `Restart` or the mode's primary verb (filled) | Existing is the opening mode when sources exist; Remove arms then confirms |
