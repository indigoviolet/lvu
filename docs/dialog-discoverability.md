...
| 8 | Sources / Add source | Existing: `Restart`/`Remove`; add modes: `Open`/`Request proposal`/`Start reviewed source` (+ `Rescan`) | none | Existing opens when sources exist and shows full status; Alt-F/Alt-C are the File/Command radios (not buttons; palette rows), Ctrl-D/Ctrl-R palette rows |
...
| 17 | Bookmarks | `Open in All events` `Edit note` `Inspect context` `Remove`; Note child `Save note` | permanent navigation and temporary inspection are named separately; Note child keeps the 1024-byte help | `Inspect c_o_ntext` retains Alt-O; Edit and Remove retain their aliases; Enter runs `Open in All events` |
