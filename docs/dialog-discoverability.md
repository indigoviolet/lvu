...
| 9 | View | Header `New _b_lank │ _C_lone │ _R_ename │ _S_ources`; action row `Apply`/`Apply membership` only. The active segment is bold and the mnemonic letter remains underlined. | unchanged | Alt-B/C/R/S switch segments; retained Alt-M/D aliases stay unlisted; Apply is the sole button/default |
