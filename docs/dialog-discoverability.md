...
| 8 | Sources / Add source | Sources: `Add source`/`Restart`/`Remove`; Add source: `Open`/`Request proposal`/`Start reviewed source` (+ `Rescan`) | none | `N` opens the highlighted-source list and `n` goes directly to creation; Alt-F/Alt-C are the File/Command radios, Ctrl-D/Ctrl-R palette rows |
