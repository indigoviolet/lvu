...
| 8 | Sources / Add source | Sources: `_A_dd source`/`_R_estart`/`Re_m_ove`; Add source: `_O_pen`/`Request _p_roposal`/`Start _r_eviewed source` (+ `_R_escan`) | the older-unavailable disclosure is a separate list row | `N` opens the highlighted-source list and `n` goes directly to creation; Alt-O/P/R reaches its visible action when no text field owns the key (Alt works from text), Alt-U expands/collapses older unavailable sources, Alt-F/Alt-C are the File/Command radios, Ctrl-D/Ctrl-R palette rows |
| 9 | View | Header `New _b_lank │ _C_lone │ _R_ename │ _S_ources │ Del_e_te`; action row `Apply`/`Apply membership`/`Delete view` only. The active segment is bold and the mnemonic letter remains underlined. Delete view carries no mnemonic: Tab plus Enter (twice: arm, confirm) or a click, never a bare letter. | unchanged, plus Delete help `All events cannot be deleted. Deleting a view cannot be undone.` | Alt-B/C/R/S/E switch segments; retained Alt-M/D aliases stay unlisted; Apply is the sole button/default except Delete, which is destructive and never filled |
| 10 | Recipes | `_A_pply`, `Sa_v_e revision`, `Re_v_iew import`, `E_x_port revision`, `Appl_y_ revision`, `_C_ancel`, plus `_S_ave`/`_U_pdate`/`_H_istory`/`_M_ore`; suggestion row `_A_dapt`/`_R_eject` | History mode: `Enter applies the selected revision; …` → `Applying a revision leaves the recipe's own pointer unchanged.` | Browse has bare mnemonic verbs; name/path fields retain bare text and accept Alt mnemonics. Alt-B/I/E/G remain mode/palette routes; `x` remains the Reject alias |
...
| 12 | Storage | `_R_efresh` `Preview _c_leanup` / `Confirm _c_leanup` (and `_M_ore` only under narrow overflow) | none | Alt-R, Alt-C, Alt-M; `r`, `c` keep working where no text field owns them |
| 13 | Settings | `_S_ave` (`_M_ore` only when the body overflows) | none | Alt-S/Alt-M; bare letters work when no field owns text |
| 14 | Help | none | — | rewritten (above) |
| 14a | Folding (landed on main during this work) | `_C_ollapse expanded runs` | column-composer message `space picks a field · Enter builds the column` → `choosing the fields that make up the new column` (the keys are the conventions) | opened from the palette only (`Folding`, `Fold repeated events`); `c`/Alt-C presses the visible action |
...
| 17 | Bookmarks | `_O_pen in All events` `_E_dit note` `Inspect c_o_ntext` `_R_emove`; Note child `_S_ave note` | permanent navigation and temporary inspection are named separately; Note child keeps the 1024-byte help | Alt-O/E/R; note input retains bare text and Alt-S saves it; Enter runs `Open in All events` |
...
| 19 | Investigation 🧠 (legacy) | `_S_tart`/`_R_esume`/`_S_end` (`_O_pen`, `New s_n_apshot`) | unchanged | Question input retains bare text; Alt reaches every visible action |
| 20 | Automatic setup status | `_A_nalyze again` `_C_lose` | existing log → state → Paseo session/creating → diagnostic | `Current log > Setup status`; the inspector is read-only unless Analyze again is selected |
