...
fn correlation_request_uses_frozen_identity_and_never_displayed_value() {
    ...
    let (generation, origin) = match &requests[0] {
        lvu::app::CorrelationRequest::Resolve {
            ...
            (*generation, origin_view_id.clone())
        }
...
fn changing_origin_view_cancels_and_fences_correlation_completion() {
    ...
    assert!(matches!(
        app.take_correlation_requests().as_slice(),
        [lvu::app::CorrelationRequest::Cancel { generation: cancelled, origin_view_id }]
...
#[test]
fn pending_correlation_freezes_picker_and_cancelled_stale_completion_releases_capacity() {
    let (provider, mut app) = demo();
    app.sync_provider(&provider, 10);
    ...
    for _ in 0..9 {
        ...
        let resolve = app
            ...
            .find_map(|request| match request {
                lvu::app::CorrelationRequest::Resolve {
                    ...
                    origin_view_id,
                    ..
                } => Some((generation, origin_view_id)),
                _ => None,
            })
        ...
        assert!(matches!(
            app.take_correlation_requests().as_slice(),
            [lvu::app::CorrelationRequest::Cancel { generation, origin_view_id }]
                if *generation == resolve.0 && origin_view_id == &resolve.1
        ));
        assert!(!app.finish_correlation(resolve.0, &resolve.1, Ok("stale".into())));
        assert_ne!(app.action_notice.as_deref(), Some("stale"));
    }
}

#[test]
fn correlation_capacity_is_bounded_and_queue_full_preserves_the_picker() {
    let (provider, mut app) = demo();
    app.sync_provider(&provider, 10);
    for _ in 0..8 {
        app.handle(Action::Open(Open::Fields), &provider);
    ...
    assert_eq!(
        app.action_notice.as_deref(),
        Some("correlation request queue is full; try again shortly")
    );
}
...
fn correlation_choices() -> Vec<lvu::CorrelationSourceChoice> {
    vec![
...
fn a_correlation_never_maps_a_differently_named_field_without_the_user_choosing_it() {
    let (provider, mut app) = demo();
    app.sync_provider(&provider, 10);
    let (generation, _) = open_correlation(&mut app, &provider);
    assert_eq!(app.focus, Focus::Correlation);
    let dialog = app.correlation_dialog.clone().unwrap();
    assert_eq!(dialog.mapped_sources(), 1);
    // `worker` carries `req`, not `request_id`. Accepting now correlates only
    // the source whose field name actually matched.
    let correlation = dialog.correlation().unwrap();
    ...
    app.handle(Action::MoveCorrelation(1), &provider);
    app.handle(Action::ActivateCorrelation, &provider);
    assert_eq!(app.correlation_dialog.as_ref().unwrap().popup, Some(0));
    app.handle(Action::MoveCorrelation(1), &provider);
    app.handle(Action::ActivateCorrelation, &provider);
    let correlation = app
        .correlation_dialog
    ...
    app.handle(Action::FocusCorrelationControl(1), &provider);
    app.handle(Action::ActivateCorrelation, &provider);
...
fn a_rejected_or_cancelled_mapping_leaves_the_origin_view_exactly_as_it_was() {
    ...
    // Escape closes the popup first, then the dialog. Neither touches the view.
    app.handle(Action::FocusCorrelationControl(-1), &provider);
    assert_eq!(
        app.correlation_dialog.as_ref().unwrap().control,
        lvu::CorrelationControl::Sources
    );
    app.handle(Action::ActivateCorrelation, &provider);
    assert!(app.correlation_dialog.as_ref().unwrap().popup.is_some());
    app.handle(Action::CancelEditor, &provider);
    assert!(app.correlation_dialog.as_ref().unwrap().popup.is_none());
    assert_eq!(app.focus, Focus::Correlation);
    app.handle(Action::CancelEditor, &provider);
    assert!(app.correlation_dialog.is_none());
    assert_eq!(app.focus, Focus::Logs);
...
#[test]
fn an_empty_mapping_is_refused_and_the_dialog_renders_its_state() {
    let (provider, mut app) = demo();
    app.sync_provider(&provider, 10);
    open_correlation(&mut app, &provider);
    // Unmap the one source that matched by name.
    app.handle(Action::ActivateCorrelation, &provider);
    app.handle(Action::MoveCorrelation(-1), &provider);
    app.handle(Action::ActivateCorrelation, &provider);
    assert_eq!(app.correlation_dialog.as_ref().unwrap().mapped_sources(), 0);
    ...
    assert!(
        screen.contains("choose the field that carries this value in at"),
        "{screen}"
    );
    assert!(screen.contains("[ Correlate ]"), "{screen}");
    assert!(!app.hit_regions.correlation_rows.is_empty());
    assert_eq!(app.hit_regions.correlation_controls.len(), 2);
}
